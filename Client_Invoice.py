...
elif tool_mode == "Bulk Invoice":
    st.write("This tool creates seperate invoices for each balance client who completed at least one order between the given dates")                                             
    today = datetime.date.today()
    tomorrow = today + datetime.timedelta(days=1)
    start_date = st.date_input('Start date', today)
    end_date = st.date_input('End date', tomorrow)
...
sql = "SELECT o.user_id, sum(o.payment) as payment, l.bank_name as parasut_id, l.address_legal_city as vergi_no, l.address_legal as address, l.address_actual as vergi_dairesi from orders o left join legal_persons l on l.user_id=o.user_id WHERE o.status_id='3' and o.finish_datetime between '"+start+"' and '"+end+"' group by user_id"
